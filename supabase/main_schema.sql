-- BabGuGu main database schema
-- Safe to run repeatedly. Destructive reset queries are kept in supabase/manual.
...
create table if not exists public.meetings (
  ...
  delivery_fee int,
  split_location text,
  baemin_together_url text,
  host_id uuid references public.users(id) on delete set null,
  host_name text,
  scheduled_at timestamp,
  created_at timestamp default now(),
  updated_at timestamp default now(),
  constraint meetings_status_check check (status in ('open', 'started', 'completed'))
);

alter table public.meetings add column if not exists status text;
update public.meetings set status = 'open' where status is null;
...
begin
  if not exists (
    ...
    check (status in ('open', 'started', 'completed'));
  end if;
end $$;

alter table public.meetings add column if not exists category text;
alter table public.meetings add column if not exists address text;
alter table public.meetings add column if not exists latitude double precision;
alter table public.meetings add column if not exists longitude double precision;
alter table public.meetings add column if not exists image_url text;
alter table public.meetings add column if not exists restaurant_id uuid references public.restaurants(id) on delete set null;
alter table public.meetings add column if not exists delivery_app text;
alter table public.meetings add column if not exists min_order_amount int;
alter table public.meetings add column if not exists delivery_fee int;
alter table public.meetings add column if not exists split_location text;
alter table public.meetings add column if not exists baemin_together_url text;
alter table public.meetings add column if not exists scheduled_at timestamp;
alter table public.meetings add column if not exists updated_at timestamp default now();
...
create table if not exists public.settlements (
  ...
  bank_info text,
  memo text,
  status text default 'requested' check (status in ('requested', 'completed', 'cancelled')),
  created_at timestamp default now(),
...
create table if not exists public.settlement_members (
  ...
  status text default 'requested' check (status in ('requested', 'paid', 'confirmed')),
  paid_at timestamp,
...
create table if not exists public.solo_place_reviews (
  id uuid primary key default gen_random_uuid(),
...
-- Prototype policy posture: most app tables are open while student OTP rows stay protected.
...
drop policy if exists "Users can update their own solo place reviews" on public.solo_place_reviews;

create policy "Solo place reviews are readable"
on public.solo_place_reviews
for select
to authenticated
using (true);

create policy "Users can create their own solo place reviews"
on public.solo_place_reviews
for insert
to authenticated
with check (auth.uid() = user_id);

create policy "Users can update their own solo place reviews"
on public.solo_place_reviews
for update
to authenticated
